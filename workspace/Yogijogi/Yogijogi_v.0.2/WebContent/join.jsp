...
<script>
	window.onload = function () {
		$("#mid").keyup(function() {
			...
			if($("#mid").val() != "") {
				...
				if($("#mid").val().length <4) {
					...
					$("#mid_mss").css("display","block");
					$("#mid_mss").text("아이디는 4글자 이상 입력 해주세요.");
					...
					$.ajax({
				        ...
				        type: "POST",
				        url: projectName+"/memberJoinOk.jsp",
				        data: param,
				        dataType: 'html',
				        beforeSend : function(){
				        	//alert("beforeSend");
				        ...
				        success: function (data) {
				        	...
				        	if(data == "TRUE") {
								...
								$("#mid_mss").css("display","none");
				            	
				        	} else if(data == "FALSE") {
		
								$("#mid_mss").css("display","block");
								$("#mid_mss").text("사용할 수 없는 아이디입니다.");
		...
		$("#mpw").keyup(function() {
			//alert($("#mid").val());
			if($("#mpw").val().length < 4) {
				$("#mpw_mss").css("display","block");
				$("#mpw_mss").text("비밀번호를 4글자 이상 입력해주세요.");
			} else {
				$("#mpw_mss").css("display","none");
			}
		});
		$("#mpwcheck").keyup(function() {
			//alert($("#mid").val());
			if($("#mpwcheck").val() != $("#mpw").val()) {
				$("#mpwcheck_mss").css("display","block");
				$("#mpwcheck_mss").text("비밀번호가 일치하지 않습니다.");
			} else {
				$("#mpwcheck_mss").css("display","none");
			}
		});
		...
		$("#memail").keyup(function() {
			...
			if($("#memail").val() != "") {
				...
				if($("#memail").val().split("@")[1] == null || $("#memail").val().split("@")[1] == "") {

					$("#memail_mss").css("display","block");
					$("#memail_mss").text("유효한 이메일 형식이 아닙니다.");
					...
					$.ajax({
				        ...
				        success: function (data) {
				        	...
				        	if(data == "TRUE") {
								...
								$("#memail_mss").css("display","none");
				            	
				        	} else if(data == "FALSE") {
	
								$("#memail_mss").css("display","block");
								$("#memail_mss").text("사용할 수 없는 이메일입니다.");
		...
		middle.on("keyup", function(){
			if ($(this).val().length == 3) {
				last.focus();
				last.attr("maxlength","5");
			}
		});
		last.on("keyup", function(){
			if ($(this).val().length == 5) {
				if (middle.val().length == 3) {
					middle.val(meddle.val() + $(this).val().substr(0,1));
					last.val($(this).val().substr(1,4));
					last.attr("maxlength","4");
				} else if (last.val().length == 5) {
					last.val($(this).val().substr(0,4));
				}
			}
		});
		*/
...
<!-- ============================ 메인 컨텐츠 영역 ================================ -->
	...
	<div class="wrap">
		<div class="login_wrap">
			<h2>회원가입</h2>
			<form id="myForm" name="join" method="post" action="<%=request.getContextPath()%>/member/joinAction.do" onsubmit="return FormCheck('join');">
				<div class="use">
					<div>전자상거래(인터넷사이버몰) 표준약관<br><br>
						표준약관 제10023호<br><br>
						제1조(목적) 이 약관은 요기조기 회사(전자상거래 사업자)가 운영하는 요기조기 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리․의무 및 책임사항을 규정함을 목적으로 합니다.<br>
						※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다.」<br>
						제2조(정의)<br><br>
							① “몰”이란 요기조기 회사가 재화 또는 용역(이하 “재화 등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 재화 등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.<br>
						...
						제14조(환급) “몰”은 이용자가 구매신청한 재화 등이 품절 등의 사유로 인도 또는 제공을 할 수 없을 때에는 지체 없이 그 사유를 이용자에게 통지하고 사전에 재화 등의 대금을 받은 경우에는 대금을 받은 날부터 3영업일 이내에 환급하거나 환급에 필요한 조치를 취합니다.<br><br>
						제15조(청약철회 등)<br><br>
				...
				<div class="phone">
					<span>전화번호</span>
					<div>
						<input id="first" type="text" name="first" maxlength="3"> - 
						<input id="middle" type="text" name="middle" maxlength="4"> - 
						<input id="last" type="text" name="last" maxlength="4">
					</div>
				</div>
				<div>
					<span>이메일</span> <input id="memail" type="email" name="memail">
					<b id="memail_mss">사용 가능한 이메일이 아닙니다.</b>
				</div>
