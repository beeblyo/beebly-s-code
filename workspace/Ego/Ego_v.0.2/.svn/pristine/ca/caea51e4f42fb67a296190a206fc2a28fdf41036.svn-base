...
<main>
	...
	<div class="content_from">
		<form>
			<a href="write.jsp?tab=4" class="write_btn btn">글 쓰기</a>
			<div class="table">
				<div class="thead">
					<div class="row">
			...
			<div class="paging">
				...
				if( startBlock >= 10 )
				{
					%><a href="notice.jsp?tab=4&kind=<%= kind %>&page=<%= startBlock - 1 %>">이전 페이지</a> | <%
				}		
				...
				if( endBlock < maxpage)
				{
					%><a href="notice.jsp?tab=4&kind=<%=kind%>&page=<%= endBlock + 1 %>">다음 페이지</a><%
				}
				%>
				</ul>
			</div>
